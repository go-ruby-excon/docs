...
`go-ruby-excon/excon` reimplements Ruby's `excon` in **pure Go, with cgo disabled**. The slice of
...
- any Go program can import `github.com/go-ruby-excon/excon` directly, with no Ruby runtime;
...
Because `excon` is pure Go with cgo disabled, it also compiles to **WebAssembly**
